"""Market and indicator chart builders."""
...
def plot_volatility(df, symbol, fast_window, slow_window, return_type="log"):
    ...
    fig.add_trace(
        go.Scatter(
            x=df["timestamp"],
            y=df[f"volatility_{fast_window}"],
            mode="lines",
            name=f"Fast Vol ({fast_window})",
    ...
    fig.add_trace(
        go.Scatter(
            x=df["timestamp"],
            y=df[f"volatility_{slow_window}"],
            mode="lines",
            name=f"Slow Vol ({slow_window})",
...
def plot_volatility_regimes(df, symbol, fast_window, slow_window):
    ...
    fig.add_hline(
        ...
        line_dash="dash",
        line_color="blue",
        annotation_text=f"Low: {low_thr:.3f}",
        row=1,
        col=1,
    )
    fig.add_hline(
        ...
        line_dash="dash",
        line_color="red",
        annotation_text=f"High: {high_thr:.3f}",
        row=1,
        col=1,
    )
    ...
    fig.update_layout(title=f"{symbol.upper()} - Volatility Regimes", height=700, showlegend=True)
    return fig
...
def plot_volatility_distribution(df, fast_window, slow_window, symbol):
    ...
    fig = make_subplots(
        ...
        cols=2,
        subplot_titles=(
            f"Fast Vol Distribution (w={fast_window})",
    ...
    fig.add_trace(
        go.Histogram(x=fast_vol, nbinsx=50, name="Fast Vol", marker_color="#d62728", opacity=0.7),
        row=1,
        col=1,
    )
    fig.add_trace(
        go.Histogram(x=slow_vol, nbinsx=50, name="Slow Vol", marker_color="#9467bd", opacity=0.7),
        row=1,
        col=2,
    )
    fig.update_layout(title=f"{symbol.upper()} - Volatility Distributions", height=500, showlegend=False)
    return fig
